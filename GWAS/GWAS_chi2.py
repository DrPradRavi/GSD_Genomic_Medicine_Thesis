...
def plot_p_value_histogram(p_values, output_dir, bonferroni_threshold):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'GWAS_p_value_histogram.png'))
    plt.close()
...
def plot_ordered_p_value_histogram(results_df, output_dir, bonferroni_threshold):
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'GWAS_manhattan_plot.png'), dpi=300)
    plt.close()
...
def main():
    print("Starting Chi-Square Feature Selection process...")
    ...
    chunk_size = X.shape[1] // num_cores
    ...
    # Combine results
    chi2_scores = np.concatenate([r[0] for r in results])
    p_values = np.concatenate([r[1] for r in results])
    ...
    results_df.to_csv(os.path.join(OUTPUT_DIR, 'GWAS_all_features.csv'), index=False)
    ...
    threshold_info.to_csv(os.path.join(OUTPUT_DIR, 'GWAS_threshold_info.csv'), index=False)
    ...
    gene_summary.to_csv(os.path.join(OUTPUT_DIR, 'GWAS_gene_summary.csv'))
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, 'GWAS_gene_distribution.png'), dpi=600, bbox_inches='tight')  # Increased DPI and added bbox_inches='tight'
    plt.close()
